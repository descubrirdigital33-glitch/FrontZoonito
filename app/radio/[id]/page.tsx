...
// ============================================================================
// COMPONENTE: REPRODUCTOR PRINCIPAL CON CONTROL DE TRANSMISIÓN MEJORADO
// ============================================================================
...
interface PlayerProps {
    ...
    isPlaying: boolean;
    isTransmitting: boolean; // NUEVO: controla si está transmitiendo
    isMicMuted: boolean;
    micVolume: number;
    musicVolume: number;
    onPlayPause: () => void;
    onToggleTransmission: () => void; // NUEVO: toggle de transmisión
    onToggleMic: () => void;
...
const Player: React.FC<PlayerProps> = ({
    ...
    const {
        ...
        isPlaying,
        isMicMuted,
        isTransmitting // NUEVO: pasar el estado de transmisión
    });
            ...
            <div className="flex flex-col gap-4">
                <div className="flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-6">
                    <div className="flex items-center gap-4 w-full md:w-auto">
                        {/* BOTÓN DE REPRODUCCIÓN LOCAL (solo música) */}
                        <button
                            onClick={onPlayPause}
                        ...
                        {/* BOTÓN DE TRANSMISIÓN (ON AIR) - Solo para el dueño */}
                        {isOwner && canTransmit && radio.isLive && (
                            <button
                                onClick={onToggleTransmission}
                                className={`w-14 h-14 md:w-16 md:h-16 rounded-full transition-all flex items-center justify-center flex-shrink-0 relative ${isTransmitting
                                    ? 'bg-red-500 hover:bg-red-600 shadow-lg shadow-red-500/50 animate-pulse'
                                    : 'bg-gray-600 hover:bg-gray-700 border-2 border-white/20'
                                    }`}
                            >
                                {isTransmitting ? (
                                    <Radio className="w-6 h-6 md:w-8 md:h-8 text-white" />
                                ) : (
                                    <Radio className="w-6 h-6 md:w-8 md:h-8 text-white/50" />
                                )}
                                {isTransmitting && (
                        ...
                        {/* BOTÓN DE MUTEAR MICRÓFONO - Solo visible cuando está transmitiendo */}
                        {isOwner && canTransmit && radio.isLive && isTransmitting && (
                            <button
                                onClick={onToggleMic}
                        ...
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                            ...
                            {isOwner && radio.isLive && (
                                <>
                                    ...
                                    {isTransmitting && isMicMuted && (
                                        <span className="bg-yellow-500 px-2 py-1 rounded text-xs font-bold text-white flex items-center gap-1">
                                            <MicOff size={12} />
                                            MIC MUTE
                                        </span>
                                    )}
                            ...
                            {!isOwner && isPlaying && isTransmitting && !streamError && (
                            ...
                            <>
                                ...
                                {canTransmit && (
                                    <button
                                        ...
                                        className={`px-3 py-1 rounded-lg text-sm font-semibold transition-colors flex items-center gap-1 ${radio.isLive
                ...
                {isOwner && isTransmitting && radio.isLive && (
                    <div className={`border rounded-lg p-3 flex items-center gap-2 ${isMicMuted ? 'bg-yellow-500/10 border-yellow-500/30' : 'bg-green-500/10 border-green-500/30'
                        }`}>
                        <div className={`w-2 h-2 rounded-full ${isMicMuted ? 'bg-yellow-500' : 'bg-green-500 animate-pulse'
                            }`} />
                        <p className={`text-sm ${isMicMuted ? 'text-yellow-300' : 'text-green-300'}`}>
                            {isMicMuted
                ...
                {isOwner && canTransmit && radio.isLive && (
                    <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-3">
                        ...
                        <ul className="text-blue-300/80 text-[10px] space-y-1 list-disc list-inside">
                            <li><strong>▶️ Play/Pause:</strong> Reproduce/pausa la música localmente (no afecta la transmisión)</li>
                            <li><strong>📡 ON AIR:</strong> Activa/desactiva la transmisión en vivo a los oyentes</li>
                            <li><strong>🎤 Mic:</strong> Silencia/activa tu micrófono (solo cuando estás ON AIR)</li>
                        </ul>
                    </div>
                )}
                ...
                {!radio.isLive && (
                    <div className="bg-gray-500/10 border border-gray-500/30 rounded-lg p-3 flex items-center gap-2">
                        ...
                        <p className="text-gray-300 text-sm">
                            {isOwner && canTransmit
                                ? 'Haz clic en "ENCENDER" para activar la radio y poder transmitir'
                ...
                <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4">
                    ...
                    {!guestCode ? (
                        ...
                        <div className="space-y-3">
                            ...
                            <button
                                onClick={onGenerateCode}
                                className="w-full bg-green-600/20 hover:bg-green-600/30 border border-green-500/50 text-green-300 py-2 rounded-lg transition-colors text-sm flex items-center justify-center gap-2"
                            >
                                <Zap size={16} />
...
const RadioSystem: React.FC = () => {
    ...
    // Handler para Play/Pause de MÚSICA (no afecta transmisión)
    const handlePlayPause = () => {
        ...
        // Solo controla la reproducción local de música
        setIsPlaying(!isPlaying);

        if (!isPlaying) {
            console.log('▶️ Reproduciendo música localmente');
        } else {
            console.log('⏸️ Música pausada localmente');
        }
    };

    // NUEVO: Handler para activar/desactivar TRANSMISIÓN
    const handleToggleTransmission = () => {
        if (!radio?.isLive) {
            Swal.fire({
                icon: 'warning',
                title: 'Radio apagada',
                text: 'Primero debes encender la radio',
                background: '#1a1a2e',
                color: '#fff',
                confirmButtonColor: '#f59e0b',
            });
        ...
        const newState = !isTransmitting;
        setIsTransmitting(newState);

        if (newState) {
            // Al activar transmisión, asegurarse de que la música esté sonando
            if (!isPlaying) {
                setIsPlaying(true);
            }
            ...
            Swal.fire({
                icon: 'success',
                title: '📡 ¡ON AIR!',
                text: 'Tu transmisión está en vivo. Los oyentes te están escuchando.',
                background: '#1a1a2e',
                color: '#fff',
                confirmButtonColor: '#ef4444',
                timer: 2500,
            ...
            Swal.fire({
                icon: 'info',
                title: '📴 OFF AIR',
                text: 'Transmisión detenida. Los oyentes no te escuchan.',
                background: '#1a1a2e',
                color: '#fff',
    ...
    // Handler para mutear/desmutear micrófono
    const handleToggleMic = () => {
        if (!isTransmitting) {
            Swal.fire({
                icon: 'warning',
                title: 'No estás transmitiendo',
                text: 'Activa la transmisión (ON AIR) primero',
                background: '#1a1a2e',
                color: '#fff',
                confirmButtonColor: '#f59e0b',
                timer: 2000,
        ...
        // El efecto en useRadioStream se encargará de cambiar el gain a 0 o 1

        Swal.fire({
            icon: newMutedState ? 'warning' : 'success',
            title: newMutedState ? '🔇 Micrófono silenciado' : '🎤 Micrófono activo',
            text: newMutedState
                ? 'Los oyentes NO te escuchan (pero sí escuchan la música)'
                : 'Los oyentes te pueden escuchar',
            background: '#1a1a2e',
            color: '#fff',
            confirmButtonColor: newMutedState ? '#eab308' : '#22c55e',
            timer: 2000,
            showConfirmButton: false
        });
    };

    // Handler para encender/apagar la radio (habilita/deshabilita transmisión)
    const handleToggleLive = async () => {
        ...
        if (updatedRadio) {
            ...
            if (newLiveState) {
                // Radio encendida
                setIsMicMuted(false);
                setIsTransmitting(false); // No transmitir automáticamente
                setIsPlaying(false);

                Swal.fire({
                    icon: 'success',
                    title: '✅ Radio encendida',
                    html: '<p>Radio lista para transmitir</p><p class="text-xs text-white/60 mt-2">Usa el botón 📡 para iniciar la transmisión en vivo</p>',
                    background: '#1a1a2e',
                    color: '#fff',
                    confirmButtonColor: '#8b5cf6',
                    timer: 3000,
                ...
                Swal.fire({
                    icon: 'info',
                    title: '⏹️ Radio apagada',
                    text: 'Tu radio está fuera de línea',
                    background: '#1a1a2e',
                    color: '#fff',
                    confirmButtonColor: '#6b7280',
                    timer: 2000,
            ...
            // Recargar estado fresco del servidor
            setTimeout(async () => {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'No se pudo cambiar el estado de la radio',
                background: '#1a1a2e',
                color: '#fff',
                confirmButtonColor: '#ef4444',
            });
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="space-y-4 md:space-y-6">
                        <Player
                            ...
                            isPlaying={isPlaying}
                            isTransmitting={isTransmitting} // NUEVO
                            isMicMuted={isMicMuted}
                            micVolume={micVolume}
                            musicVolume={musicVolume}
                            onPlayPause={handlePlayPause}
                            onToggleTransmission={handleToggleTransmission} // NUEVO
                        ...
                        <div className="grid md:grid-cols-2 gap-4 md:gap-6">
                            <MusicaPlayer
                                ...
                                isMicActive={!isMicMuted && isTransmitting} // ACTUALIZADO: considerar transmisión
                            />
