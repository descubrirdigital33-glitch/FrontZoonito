...
import { ChevronLeft, ChevronRight, Heart, Star, Search, X, Sparkles, Play, Download } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen animate-gradient-x relative overflow-x-hidden">
      ...
      <nav className="navbar sticky top-0 z-20 mb-8 bg-black/20 backdrop-blur-md">
        ...
        <div className="flex py-3 px-4 gap-2 min-w-max overflow-x-auto">
          <button
            onClick={() => setSelectedGenre('')}
            className={`px-4 py-2 text-sm font-medium rounded-full transition whitespace-nowrap ${selectedGenre === ''
          ...
          {GENRES.map((genre) => (
            <button
              ...
              className={`px-4 py-2 text-sm font-medium rounded-full transition whitespace-nowrap ${selectedGenre === genre
                  ? 'bg-pink-500 text-white'
                  : 'bg-white/10 hover:bg-white/20 text-white'
                }`}
            >
          ...
          <button className="px-5 py-2 text-sm font-semibold text-white bg-gradient-to-r from-purple-600 to-pink-600 rounded-full hover:from-purple-700 hover:to-pink-700 transition whitespace-nowrap ml-4">
            Suscribete
          </button>
        </div>
      </nav>
            ...
            <div className="relative">
              <div className="glass-card overflow-hidden rounded-2xl shadow-2xl">
                {avanceMusics[avanceSlide] && (
                  <div className="relative">
                    <div 
                      className="h-[500px] bg-cover bg-center relative group"
                    ...
                    >
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-8">
                        <div className="bg-black/80 backdrop-blur-lg rounded-2xl p-6 border border-pink-500/30">
                          <h3 className="text-4xl font-bold text-white mb-3 glow-text">
                          ...
                          <div className="flex flex-wrap gap-3 items-center">
                            <button
                              ...
                              className={`flex items-center gap-2 px-4 py-2 rounded-full font-semibold transition-all hover:scale-105 ${
                                avanceMusics[avanceSlide].likedByUser 
                                  ? 'bg-red-500 text-white' 
                                  : 'bg-white/20 text-white hover:bg-white/30'
                              }`}
                            >
                              <Heart className={`w-5 h-5 ${avanceMusics[avanceSlide].likedByUser ? 'fill-white' : ''}`} />
                              {formatLikes(avanceMusics[avanceSlide].likes ?? 0)}
                            </button>

                            <button
                              onClick={() => setShowRatingModal(avanceMusics[avanceSlide]._id)}
                              className="flex items-center gap-2 px-4 py-2 rounded-full bg-white/20 hover:bg-white/30 text-white font-semibold transition-all hover:scale-105"
                            >
                              <Star className={`w-5 h-5 ${avanceMusics[avanceSlide].userRating ? 'fill-yellow-400 text-yellow-400' : ''}`} />
                              {avanceMusics[avanceSlide].rating?.toFixed(1) ?? '0.0'}
                            </button>

                            <button
                              onClick={() => downloadMusic(avanceMusics[avanceSlide])}
                              className="flex items-center gap-2 px-4 py-2 rounded-full bg-white/20 hover:bg-green-500 text-white font-semibold transition-all hover:scale-105"
                              title="Descargar canción"
                            >
                              <Download className="w-5 h-5" />
                              Descargar
                            ...
                            <button
                              onClick={() => addToPlaylist(avanceMusics[avanceSlide])}
                              className="px-6 py-2 rounded-full bg-pink-500 hover:bg-pink-600 text-white font-bold transition-all hover:scale-105 shadow-lg"
                            >
                              ➕ Añadir a Playlist
                            </button>
                            ...
                            {avanceMusics[avanceSlide].idMusico && (
                              <Link
                                ...
                                className="px-6 py-2 rounded-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-bold shadow-lg hover:shadow-pink-400/50 transition-all duration-300 hover:scale-105 ring-1 ring-purple-400/30 hover:ring-pink-300/50"
                              >
                                Saber Más
                              </Link>
                            )}
