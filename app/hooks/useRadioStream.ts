...
//   // Referencias para el mixer con control de volumen independiente
...
//       // Cuando isMicMuted es false, el volumen debe ser 1
//       const newVolume = isMicMuted ? 0 : 1;
//       micGainNodeRef.current.gain.value = newVolume;
//       console.log(`🎤 Micrófono ${isMicMuted ? 'MUTEADO (vol=0)' : 'ACTIVO (vol=1)'}`);
//     }
//   }, [isMicMuted]);

//   // 🎙️🎵 Propietario transmite micrófono + canciones (SOLO SI isTransmitting es true)
//   useEffect(() => {
...
//         // 3️⃣ Crear nodo de ganancia para el micrófono (CRÍTICO: respetar el estado inicial)
//         const micGain = audioContext.createGain();
//         micGain.gain.value = isMicMuted ? 0 : 1; // Establecer según el estado actual
//         micGainNodeRef.current = micGain;
//         console.log(`🎤 MicGain inicializado con volumen: ${micGain.gain.value}`);

//         // 4️⃣ Capturar micrófono
...
//         // Conectar: micrófono -> gainNode (para mute) -> mixer
//         micSource.connect(micGain);
//         micGain.connect(mixer);
//         console.log('🎤 Micrófono conectado al mixer con control de mute');

//         // 5️⃣ Buscar y conectar el elemento <audio> de las canciones
...
//         // 6️⃣ Crear procesador para capturar y enviar audio mezclado
...
//   // 🎧 Oyente recibe y reproduce audio (SOLO SI isTransmitting es true)
//   useEffect(() => {
//     if (isOwner || !isPlaying || !isTransmitting) return;
