...
//   const lerpColor = (current: RGB, target: RGB, speed: number): RGB => {
//     return {
...
//       // recordCanvas.width = 1280;  // Reducido de 1920
//       // recordCanvas.height = 720;  // Reducido de 1080
//       // AUMENTAR LA RESOLUCION ESTA PARTE DE ARRIBA DEL VIDEO DESCARGADO
      
//       const recordCanvas = document.createElement('canvas');
//       recordCanvas.width = 854;   // Cambiar de 1920 a 854
//       recordCanvas.height = 480;  // Cambiar de 1080 a 480
...
const Karaoke: React.FC<KaraokeProps> = ({ currentSong, isPlaying, inlineMode = false }) => {
  ...
  useEffect(() => {
    ...
    const darkColors = [
      '#1a1a2e', '#16213e', '#0f3460', '#533483', '#2c003e',
      '#1e3a8a', '#312e81', '#4c1d95', '#701a75', '#831843',
      '#7f1d1d', '#881337', '#9f1239', '#064e3b', '#134e4a',
      '#450a0a', '#5b21b6', '#6b21a8', '#7c2d12', '#991b1b'
    ];

    const semiDarkColors = [
      '#3b82f6', '#8b5cf6', '#ec4899', '#f59e0b', '#10b981',
      '#06b6d4', '#6366f1', '#a855f7', '#d946ef', '#f43f5e',
      '#ef4444', '#f97316', '#eab308', '#84cc16', '#22c55e',
      '#14b8a6', '#0ea5e9', '#8b5cf6', '#d946ef', '#fb923c'
    ];

    const allColors = [...darkColors, ...semiDarkColors];

    let colorChangeTimer = 0;
    const colorChangeDuration = 120;
    ...
    const drawLyrics = () => {
      ...
      backgroundColorRef.current = lerpColor(backgroundColorRef.current, targetColorRef.current, 0.02);
      ...
      gradient.addColorStop(0, `rgb(${r}, ${g}, ${b})`);
      gradient.addColorStop(0.5, `rgb(${Math.round(r * 0.9)}, ${Math.round(g * 1.1)}, ${Math.round(b * 1.05)})`);
      gradient.addColorStop(1, `rgb(${Math.round(r * 0.85)}, ${Math.round(g * 0.95)}, ${Math.round(b * 1.15)})`);
